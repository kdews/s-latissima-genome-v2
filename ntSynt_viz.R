...
if (interactive()) {
  ...
  args <- c(
    ...
    "--scale", "100000000.0",
    "--no-arrow",
    "-s", paste0(outdir, "/", seqFile_no_ext, ".sequence_lengths.sorted.tsv"),
    "-l", paste0(outdir, "/", seqFile_no_ext, ".links.tsv"),
    "-p", paste0(outdir, "/", seqFile_no_ext, "_ribbon-plot"),
    "-c", paste0(outdir, "/", seqFile_no_ext, ".chrom-paint-feats.tsv"),
    "--colour_indices", paste0(outdir, "/", seqFile_no_ext, ".target_colours.tsv")
    # "--tree", paste0(outdir, "/", seqFile_no_ext, "_est-distances.nwk")
  )
} else {
...
# Convert seqids to numeric by length order
fais <- list.files(
  path = paste0(dirname(args$sequences), "/fais"),
  pattern = ".*\\.fai$",
  full.names = T
)
# Read one or more FAI files and build a lookup table
fai_lengths <- sapply(fais, function(fai_file) {
  read.csv(
    fai_file,
    sep = "\t",
    header = F,
    col.names = c("seq_id", "length", "offset", "line_bases", "line_width")
  ) %>%
  ...
  group_by(fai) %>%
  mutate(seq_rank = row_number()) %>%
  ungroup() %>%
  mutate(seq_rank = factor(seq_rank, levels = sort(unique(seq_rank)))) %>%
  select(-fai)
chr_num_key <- fai_lengths %>%
  pull(seq_rank, seq_id)
...
# Join numeric seqid ranking onto sequences
sequences <- sequences %>%
  left_join(fai_lengths)
...
# Filter out contigs
links_ntsynt <- links_ntsynt %>%
  filter(if_all(contains("seq_rank"), ~ as.numeric(.) <= 31))
filt1 <- links_ntsynt %>% distinct(seq_id) %>% pull(seq_id) %>% as.character()
filt2 <- links_ntsynt %>% distinct(seq_id2) %>% pull(seq_id2) %>% as.character()
sequences <- sequences %>%
  filter(seq_id %in% c(filt1, filt2))
painting <- painting %>%
...
make_plot <- function(links, sequences, painting, colours_df, add_scale_bar = FALSE, centromeres = FALSE, add_arrow = FALSE, haplotypes = FALSE) {
  ...
  # scale_fill_manual(values = colours, breaks = sequences_filt) +
  # scale_colour_manual(values = colours, breaks = sequences_filt) +
